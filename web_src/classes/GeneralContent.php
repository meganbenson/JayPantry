...
class GeneralContent{
    	...
    	public static function getAllLabelsDisplay($products,$title="INVENTORY",$button="addtocart", $button2=null, $button3=null){
        $count = 0;
        $divComplete = true; 
        ...
        if(is_array($products) && count($products)>0){
            ...
            foreach($products as $product){
                ...
                $html .= GeneralContent::getLabelProductDisplay($product);
                if($count%3==2){
                    $divComplete = true;
                    $html .= "</div>";
                }
    ...
    public static function getIndividualProductDisplay($product,$button,$button2,$button3){ //FIX?
        ...
        if($button=="edit")
        {
            ...
            if($button=="delete")
            {
              $jsaction = " onclick='deleteInvintoryItem({$product->productID});' "; //link function to remove item when qty = 0
              $buttonMessage = "Remove from item from inventory"; 
            }
        }else if($button=="remove")
        {
            $jsaction = " onclick='removeItem({$product->productID}, {$product->basketItemID});' ";
            $buttonMessage = "Remove from Cart";
            $qtyDisplay = $product->qty;
            if ($button2=="+") {
              $product->qty = $product->qty + 1;
              $jsaction2 = " onclick='UpdateCartQty({$product->productID}, {$product->qty});' ";
              $buttonMessage2 = "+";
            }
            if($button3== "-") {
              $product->qty = $product->qty -2;
              $jsaction3 = " onclick='UpdateCartQty({$product->productID}, {$product->qty});' ";
              $buttonMessage3 = "-";
              if ($product->qty == 0) {
                $jsaction3 = " onclick='removeItem({$product->productID}, {$product->basketItemID});' ";
              }
            }
        }
        else  {
          $button = null;
            ...
            $jsaction = " onclick='addItem({$product->productID}, 1);' ";
        ...
        if ($button != null and $button2 != null) {
            $display .= "<button class='w3-button w3-red' $jsaction2 type='button'>{$buttonMessage2}</button>";
            $display .= "<button class='w3-button w3-red' $jsaction3 type='button'>{$buttonMessage3}</button>";
        }
        $display .= "<p>{$product->productName}<br>";
        $display .= " Qty: {$qtyDisplay}</p>";
        $display .= " EXP: {$product->expirationDate}</p>";
        $display .= "</div>";
        $display .= "</div>";
        return $display;    
    }

    public static function getLabelProductDisplay($product){ //FIX?

       
	$display = "<img src='barcode.php?f=png&s=code-39&d=".$product->productID."'>";
        $display .= "<p>{$product->productName}</p>";
	$display .= "<BR><BR><BR>";
    ...
    public static function getLabelsButton(){
      $display = "<div class='w3-margin-bottom w3-center'>";
      $jsaction = " onclick='getLabels();' ";
      $buttonMessage = "Get Labels";
      $display .= "<button class='w3-button w3-red' $jsaction type='button'>{$buttonMessage}</button>";
      $display .= "</div>";
      return $display;   
    }

    public static function getCheckoutButton($cartID){
      $display = "<div class='w3-margin-bottom w3-center'>";
      $display .= "<form method='post' action='index.php' class='w3-form' enctype='multipart/form-data'>";
      $jsaction = " onclick='checkoutCart($cartID);' ";
      $buttonMessage = "Checkout";
      $display .= "<input type='hidden' id='id' name='id' value='{$cartID}'>";
      $display .= "<button class='w3-button w3-red 'name='checkoutBtn' $jsaction type='button'>{$buttonMessage}</button>";
      $display .= "</form>";
      $display .= "</div>";
      return $display;   
    }
    public static function getUseScannerButton($cartID){
      $display = "<div class='w3-margin-bottom w3-center'>";
      $display .= "<form method='post' action='index.php' class='w3-form' enctype='multipart/form-data'>";
      $jsaction = " onclick='goToScannerPage($cartID);' ";
      $buttonMessage = "Use Handheld Scanner";
      $display .= "<input type='hidden' id='id' name='id' value='{$cartID}'>";
      $display .= "<button class='w3-button w3-red 'name='scannerBtn' $jsaction type='button'>{$buttonMessage}</button>";
      $display .= "</form>";
      $display .= "</div>";
      return $display;   
    }

    public static function getKioskWelcome($message="Scan your student ID to get started."){
      $display = "<div class='w3-margin-bottom w3-center'>";
      $display .= '<img src="images/BlueJayPantry-Logo-v4.png" style="width:30%;margin:20px;">';
      $display .= "<h2>".$message."</h2>";
      $display .= "</div>";
      return $display;   
    }
    public static function getKioskThanks(){
      $jsaction = " onclick='goToKiosk();' ";
      $buttonMessage = "Reset Scanner";
      $display = "<div class='w3-margin-bottom w3-center'>";
      $display .= '<img src="images/BlueJayPantry-Logo-v4.png" style="width:30%;margin:20px;">';
      $display .= "<h1>Thanks for using the Pantry Kiosk!</h1>";
      $display .= "<button class='w3-button w3-red 'name='scannerBtn' $jsaction type='button'>{$buttonMessage}</button>";
      $display .= "</div>";
      return $display;   
    }
...
<div class="w3-container" style="padding:128px 16px" id="about">
  ...
  <img  src="images/BlueJayPantry-Logo-v4.png" style="width:30%;margin:20px;">
  </div>
  <div class="w3-col m6">
...
<div class="w3-container w3-light-grey" style="padding:128px 16px" id="contact">
  ...
  <div style="margin-top:48px">
        ...
        switch($type){
          ...
          case 'ByDependentInfo': 
            ...
            case 'ByProduct':
                  ...
                  break;
            case 'TempData':
                  $content .= GoogleChartDisplay::getByDateReportForTemp($data);
                  break;
    ...
    public static function getRegisterForm($studentID="") {
      $error = (isset($_SESSION["error"])) ? $_SESSION["error"] : "";
      ...
      if($studentID==""){
        ...
        $formHTML .=
              '<label id="usernameLabel">Student ID:</label><br>'.$studentID.'
              <input type="hidden" id="username" name="username" value="'.$studentID.'">
              <input type="hidden" id="fromKiosk" name="fromKiosk" value="YES">';

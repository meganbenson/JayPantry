...
class PageRouter{
 ...
 //                   $web_string2 = file_get_contents($url."/data_src/api/category/read.php?APIKEY=$api_key");
                    ...
                    $content .= EditItemForm::getEditForm($products[0],$categories); 
            ...
            case "labels":
                ...
                //$web_string = file_get_contents($url."/data_src/data.php?table=products&catID={$catID}");
                $products = json_decode($web_string);
                $content .= GeneralContent::getAllLabelsDisplay($products,"LABELS"); 
                $useFoodTabs = false;
            break;


            case "reports":
                //receive the two get values and then use the api to get the data 
                //into web_string and then convert it to the correct json format 
                $catID = isset($_GET["catID"])?$_GET["catID"]:"";
                ...
                if($web_string==""){
			$content .= "<div class='w3-container w3-light-grey' style='padding:128px 16px;'>No Data Found</div>"; 
                }else{

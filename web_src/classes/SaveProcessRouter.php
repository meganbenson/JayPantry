...
class SaveProcessRouter {
    ...
    public static function processData(){
        ...
        $deletingItem = isset($_POST["deleteBtn"]) && $_POST["deleteBtn"] == "Delete Product" ? true : false;
        SaveProcessRouter::dataProcessing2($deletingItem);
    ...
    public static function dataProcessing($savingItem){
        ...
        if($savingItem){
            ...
            /*else if($_POST["id"]==""){
                $dataReturned = EditItemForm::validateAndDeleteData($_POST, $_POST["id"], $url);
            } */
            else{
                $dataReturned = EditItemForm::validateAndEditData($_POST,$_FILES,$url);
            }
            $dataDecoded = json_decode($dataReturned);
            if ($dataDecoded->message == null) {
                echo ("Error: No category specified!");
            }
            else if($dataDecoded->message!=""){
    ...
    public static function dataProcessing2($deletingItem){
        global $page,$content,$url;
        $dataReturned = "";
        if($deletingItem){ 
            $dataReturned = EditItemForm::validateAndDeleteData($_POST, $url); //Could be this line?
            $dataDecoded = json_decode($dataReturned);

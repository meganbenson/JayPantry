...
$params = [":id" => $id, ":basket_id" => $basket_id, ":quantity" => $quantity];
...
try {
    ...
    $data = FoodDatabase::getDataFromSQL($sqlCheck, [":id" => $id, ":basket_id" => $basket_id]);
        ...
        if ($data !== false) {
            ...
            $sqlInsert = "INSERT INTO basketItem (productID, BasketID, qty) 
            SELECT :id, :basket_id, 1 
            WHERE NOT EXISTS (SELECT 1 FROM basketItem WHERE productID = :id 
            and BasketID = :basket_id)";

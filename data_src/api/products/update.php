...
if($img==""){
    $params = [":name"=>$productName,":cid"=>$catID,":q"=>$qty,":id"=>$id, ":expirationDate"=>$expirationDate];
    $sql = "update product set productName=:name,catID=:cid,quantity=:q,expirationDate=:expirationDate WHERE productID=:id;";
    
}else{
    $params = [":name"=>$productName,":cid"=>$catID,":q"=>$qty,":i"=>$img,":id"=>$id];
    $sql = "update product set productName=:name,catID=:cid,quantity=:q,img=:i,expirationDate=:expirationDate WHERE productID=:id;";
